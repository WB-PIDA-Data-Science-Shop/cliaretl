...
#'
#' @param db_variables A data frame containing a column named `variable` with the list of valid variable names.
...
#'
#' @export
flag_missing_indicators <- function(db_variables, df, source_type = NULL, source_colname = "source") {
  requireNamespace("tibble", quietly = TRUE)
  requireNamespace("dplyr", quietly = TRUE)

  # Filter dictionary if source_type is provided and source_colname exists
  dictionary <- if (!is.null(source_type) && source_colname %in% colnames(db_variables)) {
    db_variables |>
...
#' Update Variable Names in a Data Frame
#'
...
#'
#' @param data A data frame containing a 'variable' column (e.g., `db_variables`).
...
update_db_variables <- function(data, rename_map = NULL, find_var_name = NULL) {
  if (!is.data.frame(data)) {
    stop("`data` must be a data frame.")
  }
  if (!"variable" %in% names(data)) {
    stop("The provided data frame must contain a 'variable' column.")
  }
  ...
  if (!is.null(rename_map)) {
    ...
    match_idx <- match(data$variable, unname(rename_map))
    data$variable[!is.na(match_idx)] <- names(rename_map)[na.omit(match_idx)]
    message("Renaming applied to the 'variable' column.")
  }
  ...
  if (!is.null(find_var_name)) {
    ...
    idx <- which(data$variable == find_var_name)
    if (length(idx) > 0) {
